...
# Test: Redness
URL=https://api-service-andersonmartins-mbciet.cloud.okteto.net/ready
...
URL=https://api-service-andersonmartins-mbciet.cloud.okteto.net/celsius/10/fahrenheit
STATUSCODE=$(curl -s -o /dev/null -w "%{http_code}" -X GET $URL -H  "accept: application/json")
if test $STATUSCODE != 200; then
    exit 1
...
URL=https://api-service-andersonmartins-mbciet.cloud.okteto.net/fahrenheit/10/celsius
STATUSCODE=$(curl -s -o /dev/null -w "%{http_code}" -X GET $URL -H  "accept: application/json")
if test $STATUSCODE != 200; then
    exit 1
    echo $URL " | HTTP Code: " $STATUSCODE " | Test Status Fail"
